...
def calculate_text_similarity(data: pd.DataFrame,
                              matrix: sp.csr_matrix, 
                              attribute_name: str) -> pd.DataFrame:
    """
    Функция для подсчета косинусной схожести.
    
    Args:
        data: Датафрейм с признаками.
        matrix: TF-IDF матрица для названий и аннотаций.
        atribute_name: Название/Аннотация. 
    """
    ...
    data = (
        data
        .merge(pairs, on=['item_cat', 'jointitem_cat'], how='left')
        .drop(['item_cat', 'jointitem_cat'], axis=1)
        .drop_duplicates()
        .reset_index(drop=True)
    )
